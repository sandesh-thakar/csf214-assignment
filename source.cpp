...
class Part1
{
	...
	/// converts infix propositional logic expression to postfix propositional logic expression 
	string infix_to_postfix(string infix) 
	{
		/// array to store precedence of operators
		...
		for(int i=0;i<infix.length();i++)                                         //Assigning precedence to operators
		{
		    ...
		    else if(infix[i]=='(') //precedence incremented by one if left bracket is encountered
		        level++;
		        
		   else if(infix[i]==')') //precedence decremented by one if right bracket is encountered
		        level--;
		}
		...
		{
			if(infix[i]=='~' || infix[i]=='^' || infix[i]=='V' || infix[i]=='>') //element is an operator
			{
			...
			else if( (infix[i]>='a' && infix[i]<='z') || (infix[i]>='A') && (infix[i]<='Z')) //element is an operand
			postfix+=infix[i];
		}
	...
	///converts postfix propositional logic expression to rooted binary parse tree 
